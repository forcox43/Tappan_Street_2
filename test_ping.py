...
from trading_functions import get_case, get_trader_info, get_assets, get_securities, get_limits_tracker, get_securities_hist, get_orders, get_securities_book, post_order, get_orderss, get_time_and_sales
...
i = .5
limit_pull = 100
flip_rate = .0125 #percent arbitrage installed in positions submitted to unravel each prior position created.
flip_qty = 250
...
while i > 0:
    ...
    if current_info_pd['position'].item() > limit_threshold: 
        ...
        if ords.empty == True: 
            ...
            post_order(stock, 'LIMIT', qty, 'SELL', prc)
        else:
        ...
        if ords.empty == True: 
            ...
            print (prc)
            #ticker, order_type, quantity, side, price
            post_order(stock, 'LIMIT', -1*qty, 'BUY', prc)
        else:
        ...
        if ords.empty == True:
            ...
            if (current_info_pd['BD_spd'].item() > -0.15 and current_info_pd['BD_spd'].item() < -0.03): #determines spread is narrow and liquid enough.
                ...
                if abs(ask_VWAP_mean - cl) < .75:    #if the mean of VWAP ask/sell is within a margin of the last close price.
                    ...
                    post_order(stock, 'LIMIT', flip_qty, 'BUY', ask_VWAP_mean * (1 - flip_rate)) 
